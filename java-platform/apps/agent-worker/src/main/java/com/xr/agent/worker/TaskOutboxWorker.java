...
public final class TaskOutboxWorker {
    ...
    public TaskOutboxWorker(OutboxStorePort outboxStore, TaskPersistencePort taskPersistence) {
        this(outboxStore, taskPersistence, Clock.systemUTC(), DEFAULT_PROCESSING_LEASE, DEFAULT_RETRY_DELAY);
    }
    ...
    private void process(TaskOutboxMessage message, Instant now) {
        ...
        updateWithIdempotentConflictRecovery(task, message);
    }

    private void updateWithIdempotentConflictRecovery(AgentTask task, TaskOutboxMessage message) {
        try {
            ...
            if (current.status() == TaskStatus.CREATED) {
                throw new EventProcessingException("TASK_VERSION_CONFLICT");
            }
        }
    }
